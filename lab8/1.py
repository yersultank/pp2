...
class Enemy(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.bottom>600: #if enemy car passes the players car
            score+=1 #score++
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.left>0:
            if pressed[K_LEFT]:
                self.rect.move_ip(-20,0) #if left keyboard is pressed move the car by 20 pixels left by x-axis (negative 20 because coordinates start from top left corner)
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(self.image, (40, 40)) #make the size of the coin 40x40 bcz the original is too big
        self.rect=self.image.get_rect() #get a rectangle of image
        self.rect.center=(random.randint(40,width-40),520) #place it randomly by x-axis the same line with player's car
...
monety.add(C1) #created group and added C1 to manage enemy coins
all_sprites.add(E1)
all_sprites.add(P1)
all_sprites.add(C1) #group containing every object of the game to manage them
inc_speed=pygame.USEREVENT+1
pygame.time.set_timer(inc_speed,1000) #increase of speed happening every 1 second

done=True
while done:
    for event in pygame.event.get():
        if event.type==inc_speed:
            speed+=0.5 #increase enemy cars speed every second by 0.5
        ...
        if event.type == pygame.USEREVENT + 2: #triggers when timer in Coin(move) reaches 0.5 seconds
            ...
            all_sprites.add(new_coin) #add new coins to the all_sprites group
    ...
    if pygame.sprite.spritecollideany(P1, enemies): #if player's car hits the enemy car
        pygame.mixer.Sound("crash.wav").play() #play this sound effect 
